...
function ComboSelectField({
    ...
    const includedKeys = new Set(["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight"]);
    return (
        <FormField
            ...
            render={({ field }) => {
                const getLabel: string | null = () => {
                    const selected = field.value ? options?.find((option) => option[valueProp] === field.value) : null;
                    if (selected) {
                        return selected[labelProp] as string;
                    }
                    return placeholder;
                };
                return (
                            ...
                            <PopoverTrigger asChild>
                                <FormControl>
                                    <Button
                                        onKeyDown={(e) => {
                                            e.stopPropagation();
                                            if (includedKeys.has(e.code) || alphabets.has(e.key)) setOpen(true);
                                        }}
                                        variant="outline"
                                        role="combobox"
                                        aria-expanded={open}
                                        ...
                                        <CommandGroup>
                                            {options.map((option) => (
                                                <CommandItem
                                                    ...
                                                    onSelect={() => {
                                                        form.setValue(name, option[valueProp]);
                                                        setOpen(false);
                                                    }}>
                                                    <CheckIcon
                                                        className={cn(
                                                            "mr-2 h-4 w-4",
                                                            option[valueProp] === field.value
                                                                ? "opacity-100"
                                                                : "opacity-0",
                                                        )}
                                                    />
