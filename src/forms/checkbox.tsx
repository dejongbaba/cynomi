...
function CheckboxField({ name, control, label, description }: ComponentProps<FieldValues>) {
    return (
        <FormField
            control={control}
            name={name}
            render={({ field }) => (
                <FormItem className="flex item-start space-x-4 space-y-0">
                    <FormControl
                        ...
                        <Checkbox className="mt-1.5" checked={field.value} onCheckedChange={field.onChange} />
                    </FormControl>
